...
type Response = {fields:Setting[];secret_storage:{name:string;available:boolean;persistent:boolean}};
export default function EnvSettings({onSettingsSaved}:{onSettingsSaved?:(fields:Setting[])=>void}) {
  ...
  useEffect(()=>{let live=true;request().then(r=>{if(live){setFields(r.fields);setSecretStorage(r.secret_storage);}}).catch(e=>{if(live)setError(e.message);});return()=>{live=false;};},[]);
  ...
  return <section><h2>Environment settings</h2><p>Settings from .env.template and additional settings used by the Python code.</p><p>Precedence: saved settings → process environment → .env → default value. The .env file is not modified.</p><p>Secret storage: {secretStorage?.name??'detecting…'}{secretStorage&&!secretStorage.available?' — persistent saving is unavailable; use environment variables or a container secret mount.':''}</p><p>AI runs use LLM_BASE_URL (the API address, including /v1), LLM_API_KEY, AGENT_NODE_MODEL, and AGENT_NODE_TEMPERATURE. OpenAI-compatible Chat Completions endpoints are supported. HTTPS is required except for HTTP on localhost. A local server does not require a key. OPENROUTER_API_KEY is used only for the standard OpenRouter address when LLM_API_KEY is not set. When changing providers, replace or disable LLM_API_KEY and set its model ID.</p>
    {error&&<p role="alert">{error}</p>}{message&&<p role="status">{message}</p>}
    <form onSubmit={e=>{e.preventDefault();void commit(draft);}}>
    {Array.from(new Set(fields.filter(f=>f.group!=='API keys').map(f=>f.group))).map(group=><fieldset key={group} disabled={busy} style={{border:'1px solid #dde4ef',borderRadius:8,margin:'16px 0',padding:16}}><legend>{group}</legend><div className="two-col">{fields.filter(f=>f.group===group).map(f=><div key={f.name}><label className="field"><span>{f.name} <span className="tag">{f.source}{f.kind==='secret'?f.configured?' · configured':' · not configured':''}</span></span><input type={f.kind==='secret'?'password':f.kind==='temperature'||f.kind==='port'?'number':'text'} autoComplete="off" spellCheck={false} min={f.kind==='port'?1:f.kind==='temperature'?0:undefined} max={f.kind==='port'?65535:f.kind==='temperature'?2:undefined} step={f.kind==='temperature'?'any':undefined} value={draft[f.name]??(f.kind==='secret'?'':f.value??'')} placeholder={f.kind==='secret'?'Enter a new secret to replace it':''} onChange={e=>setDraft(d=>({...d,[f.name]:e.target.value}))}/></label>
    {f.kind==='secret'&&<button type="button" disabled={!f.configured} onClick={()=>void commit({[f.name]:''})}>Disable secret</button>}
    <button type="button" disabled={!f.overridden} onClick={()=>void commit({},[f.name])}>Use env / default</button></div>)}</div></fieldset>)}
    <button type="submit" disabled={busy||Object.keys(draft).length===0}>{busy?'Saving…':'Save changes'}</button>
