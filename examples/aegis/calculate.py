    ...
    for data in records:
        ...
        if not pred_data:
            ...
            if isinstance(model_detection, dict):
                ...
                for key in final_answer_keys:
                    if key in model_detection and isinstance(
                        model_detection[key], dict
                    ):
        ...
        pred_global_set = set(
            (d.get("agent_name"), d.get("error_type"))
            for d in pred_data
            if isinstance(d, dict)
        )
        pred_agent_set = set(
            d.get("agent_name") for d in pred_data if isinstance(d, dict)
        )
        pred_error_set = set(
            d.get("error_type") for d in pred_data if isinstance(d, dict)
        )
        ...
        analysis_entry = {
            ...
            "parsing_success": len(pred_data) > 0
            or len(gt_data) == 0,  # 是否成功解析出结果
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file_path",
        type=str,
        default="examples/aegis/results/aegis_eval_summaries_fixed",
