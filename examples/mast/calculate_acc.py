...
def compute_metrics_from_directory(directory: str) -> tuple[Dict, int]:
    ...
    results = defaultdict(lambda: {'preds': [], 'gts': [], 'tp': 0, 'fp': 0, 'tn': 0, 'fn': 0})
...
def save_to_csv(metrics: Dict, csv_path: Path):
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
        ...
        sorted_metrics = dict(sorted(metrics.items(), key=lambda x: tuple(map(float, x[0].split('.')))))
...
def print_results_table(metrics: Dict, total_files: int):
    ...
    for fm_key, m in sorted_metrics.items():
        print(f"{fm_key:<4} {m['accuracy']:<6.3f} {m['f1_score']:<5.3f} "
...
if __name__ == "__main__":
    main("/Users/alina/Desktop/ITMO/AutoJudge/examples/mast/results/mast_human_dataset_eval")
