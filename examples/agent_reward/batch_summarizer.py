...
async def main(save_folder: str, df):
    ...
    for idx in range(len(df)):
        logger.info(
            f"Processing task {idx + 1}/{len(df)}: {df.iloc[idx]['question_ID']}"
        )
        summary_file = (
            Path(__file__).resolve().parent
            / "step_summaries_gemini_2.5_flash"
            / "hand"
            / f"{df.iloc[idx]['question_ID']}.json"
        )
        if os.path.exists(summary_file):
            logger.info(
                f"Trace summary already exists for {df.iloc[idx]['question_ID']}, skipping..."
            )
            ...
            for i in range(0, len(df.iloc[idx]["history"]), chunk_size):
                chunk = df.iloc[idx]["history"][i : i + chunk_size]
                chunk_ids = [
                    f"{df.iloc[idx]['question_ID']}_{j}"
