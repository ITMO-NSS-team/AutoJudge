export function parseDesignFile(kind: 'schema' | 'taxonomy', input: string): string {
  ...
  if (!text) throw Error('Файл пуст.');
  if (kind === 'taxonomy') return text;
  const schema = JSON.parse(text);
  if (!schema || Array.isArray(schema) || schema.type !== 'object') throw Error('Output schema должна описывать JSON object.');
  const check = (value: unknown): void => {
    if (value && typeof value === 'object') for (const [key, child] of Object.entries(value)) {
      if (['$ref', '$dynamicRef', '$recursiveRef'].includes(key)) throw Error('Ссылки в schema пока не поддерживаются runner.');
      check(child);
    }
  };
  check(schema);
...
export function normalizeTrace(text: string) {
  ...
  if (!Array.isArray(arr) || !arr.length) throw Error('Ожидается массив, объект steps/turns/messages или JSONL.');
  ...
  if (normalized.some(s => !Number.isFinite(s.id)) || new Set(normalized.map(s => s.id)).size !== normalized.length) throw Error('ID шагов должны быть числовыми и уникальными.');
  return normalized;
}
