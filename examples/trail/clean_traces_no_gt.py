...
class TraceCleaner:
                ...
                if "body" in log:
                    ...
                    if "function.name" in body:
                        ...
                        if "function.arguments" in body and body["function.arguments"]:
                            ...
                            func_data["args"] = self.clean_data(
                                body["function.arguments"]
                            )
                        if "function.output" in body and body[
                            "function.output"
                        ] not in ["<null>", None]:
                            # Clean the output to remove annotations
                            func_data["output"] = self.clean_data(
    ...
    def clean_trace(self, trace_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        cleaned_trace = {"trace_id": trace_data["trace_id"]}
...
def clean_directory(input_dir: str, output_dir: str, pattern: str = "*.json"):
    ...
    print(
        f"  Total reduction:     {total_original - total_cleaned:,} bytes "
