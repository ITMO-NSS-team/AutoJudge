...
for file in files:
    ...
    if "is_corrected" in row.keys():
        ...
        print(
            f"Warning: no is_correct/is_corrected column for {question_id}, skipping..."
        )
        continue
    ...
    if pred == 0 and score == "poor":
                    ...
                    for score in data[llm_metric]["scores"]:
                        if score["score"] == "poor" or score["score"] == "fair":
                            if score["justification"]:
                                if mistake_agent_true.lower().replace(" ", "") in score[
                                    "justification"
                                ].lower().replace(" ", ""):
        ...
        if mistake_agent_true.lower().replace(
            " ", ""
        ) in judge_justification.lower().replace(" ", ""):
...
who_judge_accuracy = (
    correct_mistake_agent_judge / processed_files if processed_files > 0 else 0
)
